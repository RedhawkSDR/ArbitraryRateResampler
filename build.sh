...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e ArbitraryRateResampler.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/ArbitraryRateResampler-1.0.0
        tar czf ${tmpdir}/ArbitraryRateResampler-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} ArbitraryRateResampler-1.0.0
        rpmbuild -ta ${tmpdir}/ArbitraryRateResampler-1.0.0.tar.gz
        rm -rf $tmpdir
    else
